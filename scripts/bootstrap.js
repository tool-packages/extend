...
const currentVersion = require(resolveRoot('package.json')).version; // 当前主版本版本号

const args = minimist(process.argv.slice(2)); // 命令行参数解析规则
const targets = args._.length ? args._ : allTargets; // 目标项目，如果 packages 内没有，则会新建，默认所有
const isForce = args.force || args.f; // 强制格式化，如果包含此参数则会强制处理 package.json 等文件
const isPrivate = args.private || args.p; // 是否定义为私有包
        ...
        : // 公共包
          {
            ...
            // 项目关键字
            keywords: ['typescript', 'javascript', 'roshin', name],
            // 项目开发者
            author: {
              name: 'Roshin',
            ...
            repository: {
              type: 'git',
              url: 'https://github.com/tool-packages/extend.git'
            },
            // bug 提交地址
            bugs: {
              url: 'https://github.com/tool-packages/extend/issues'
            },
            // 项目包的官网
            homepage: `https://g.extend.roshin.cn`
          })
    };
...
`.trim() + '\n'
  ...
  // .npmignore
  const npmignorePath = resolveTarget(target, `.npmignore`);
  if (!isPrivate && (isForce || !fse.existsSync(npmignorePath))) {
    fse.outputFileSync(
      npmignorePath,
      `
# 不上传给npm的文件
