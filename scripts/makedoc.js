...
// @ts-ignore
const { FrontMatterComponent } = require('../temp/makedoc/components/front-matter'); // 文档生成工具
// const { FrontMatterComponent } = require('typedoc-plugin-markdown/dist/components/front-matter'); // 文档生成工具
...
async function makeDocs(_target) {
  ...
  app.bootstrap({
    ...
    disableSources: false, // 隐藏页底的全局链接
    media: '', // 包含媒体
    excludeTags: [], // 忽略的标签
    readme: '', // 应在索引页面上显示的自述文件的路径, 通过 none 以禁用索引页面并在 globals 页面上启动文档
    // theme: 'markdown',
    theme: resolveRoot('./temp/makedoc/theme'), // 指定应使用的主题的路径
